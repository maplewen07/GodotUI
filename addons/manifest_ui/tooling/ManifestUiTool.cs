...
public static class ManifestUiTool
{
    ...
    private static class SceneGenerator
    {
        public static string Generate(ManifestPackage package)
        {
            ...
            var sb = new StringBuilder();
            sb.AppendLine($"[gd_scene load_steps={2 + sceneResources.Count + scriptResources.Count} format=3]");
            sb.AppendLine();
            ...
            sb.AppendLine();
            EmitNode(sb, package.Layout.Root, parentPath: "", nodePath: ".", sceneResources, scriptResources);
            return sb.ToString();
        }
            ...
            Dictionary<string, string> scriptResources)
        {
            ...
            foreach (var child in node.Children)
            {
                ...
                EmitNode(sb, child, nodePath, childPath, sceneResources, scriptResources);
            }
        }
    ...
    private static class CodeGenerator
    {
        public static string Generate(ManifestPackage package)
        {
            ...
            foreach (var evt in events)
            {
                ...
                sb.AppendLine($"        {SignalConnectLine(target.FieldName, evt.Signal, Names.EventMethod(evt.EventId, evt.Signal))}");
            }
            ...
            if (events.Count > 0 || inputs.Count > 0)
            {
                ...
                {
                    var target = targets[evt.Target];
                    sb.AppendLine($"        {SignalDisconnectLine(target.FieldName, evt.Signal, Names.EventMethod(evt.EventId, evt.Signal))}");
                }
                for (var index = 0; index < inputs.Count; index++)
                {
                    var input = inputs[index];
        ...
        {
            return SignalConnectLine(target.FieldName, InputSignalName(input), InputHandlerMethod(input, index));
        }

        private static string InputDisconnectLine(Target target, InputBinding input, int index)
        {
            return SignalDisconnectLine(target.FieldName, InputSignalName(input), InputHandlerMethod(input, index));
        }
