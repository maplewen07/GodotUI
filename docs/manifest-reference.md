...
Property values currently emit strings, numbers, booleans, and two-number
arrays (as `Vector2`). Use Godot's serialized property names, such as
`custom_minimum_size`, `offset_left`, or `placeholder_text`.
