...
- Moved `LiquidGlassStyle` into its matching C# source file and enabled it as
  a tool script so Godot can instantiate typed `.tres` styles in both the
  editor and runtime instead of falling back to a plain `Resource`.
